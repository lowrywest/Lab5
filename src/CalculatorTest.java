...
public class CalculatorTest {
	...
	public void calculateTwoTokensTestInvalidCommand() throws AssertException
	{
		// TODO: complete this test...
		try {
			Calculator.calculateTwoTokens(new String[] { "foo", "8" });
			Assert.fail("Illegal expression did not throw an Exception");
		} catch (NumberFormatException e) {
			// We expect the function to throw a NumberFormatException (from failure of
			// Integer.parseInt)
	...
	{
		try {
			int result = Calculator.calculateThreeTokens(new String[] { "7", "+", "5" });
	...
	{
		try {
			int result = Calculator.execute(new String[] { "7","+", "5" });
			Assert.assertEquals(12, result);
		} catch (Exception e) {
			Assert.fail("Legal expression threw an Exception: " + e.getMessage());
		}
	...
	{
		try {
			Calculator.execute(new String[] { "6 foo 6" });
			Assert.fail("Illegal expression did not throw an Exception");
	...
	public void executeTestInvalidTokenLength() throws AssertException 
	{
		try {
			...
			// We expect the function to throw a CalculatorException.
			// Check to make sure the CalculatorException has the correct message and type:
			Assert.assertEquals("Invalid Token Length", e.getMessage());
		} catch (Exception e) {
			Assert.fail("Unexpected Exception (not CalculatorException) caught");
		}
		// Token length is 0:
		// TODO: complete this test...
		try {
			Calculator.execute(new String[] { "8","+","5","+","4" });
			Assert.fail("Illegal expression did not throw an Exception");
		} catch (CalculatorException e) {
			// We expect the function to throw a CalculatorException.
			// Check to make sure the CalculatorException has the correct message and type:
	 ...
	 */
	public void parseAndExecuteTestValidQuit() throws AssertException, NumberFormatException, CalculatorException 
	{
		String result = Calculator.parseAndExecute("quit");
		Assert.assertEquals("quit", result);
	}
	 ...
	 * @throws NumberFormatException
	 */
	public void parseAndExecuteTestValidCommand() throws AssertException, NumberFormatException, CalculatorException 
	{
	 ...
	 * @throws NumberFormatException
	 */
	public void parseAndExecuteTestInvalidCommand() throws AssertException, NumberFormatException, CalculatorException {
		String result = Calculator.parseAndExecute("6 foo 6");
	 ...
	 * @throws NumberFormatException 
	 */
	public void parseAndExecuteTestInvalidTokenLength() throws AssertException, NumberFormatException, CalculatorException {
		// TODO: complete this test...
